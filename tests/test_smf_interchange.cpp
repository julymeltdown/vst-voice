...
TEST_CASE("SMF import reports nonlyric unmatched and extra lyric events separately") {
  ...
  CHECK(issues[1].message.find("no matching note onset") != std::string::npos);
  CHECK(issues[2].tick == time::Tick{0});
  CHECK(issues[2].message.find("extra lyric events") != std::string::npos);
}

TEST_CASE("SMF decoder and import disclose flattened multiple track structure") {
  using namespace seam;
  const std::vector<std::uint8_t> first{
      0U, 0x90U, 60U, 100U, 0x83U, 0x60U, 0x80U, 60U, 0U,
      0U, 0xffU, 0x2fU, 0U};
  const std::vector<std::uint8_t> second{
      0x83U, 0x60U, 0x90U, 64U, 100U, 0x83U, 0x60U, 0x80U, 64U, 0U,
      0U, 0xffU, 0x2fU, 0U};
  ...
  CHECK(decoded.value().issues.front().tick == time::Tick{0});
  CHECK(decoded.value().issues.front().message.find("2 source tracks") != std::string::npos);
  ...
  CHECK(imported.value().score.issues == decoded.value().issues);
}
