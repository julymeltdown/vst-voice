...
class Phase13AContractTests(unittest.TestCase):
    def test_dependency_lock_pins_exact_audited_revisions(self):
        ...
        self.assertEqual(
            "35f524b771ec09f54c164720bb90f271273b37d3", commits["clap-wrapper"]
        )
        self.assertEqual("3cdf9ca5d1f5b1b21e0a86832aa4abe55607bd96", commits["vst3sdk"])
        self.assertEqual(
            "bd98b31feff57a15989fcfab4cd86dc63382b1ac", commits["AudioUnitSDK"]
        )

    def test_mandatory_doc_explicitly_blocks_beta_rc_and_ga(self):
        text = (ROOT / "docs/phase13a/MANDATORY_VALIDATION_KO.md").read_text(
            encoding="utf-8"
        )
        for phrase in (
            "필수",
    ...
    def test_release_gate_is_connected_to_phase13a_matrix(self):
        matrix = json.loads(
            (ROOT / "docs/phase13a/mandatory-validation-matrix.json").read_text(
                encoding="utf-8"
            )
        )
        ids = {item["id"] for item in matrix["targets"]}
        required = {
            "linux-vst3-validator",
            "windows-vst3-validator",
            "macos-vst3-validator",
            "macos-auval",
            "windows-installer-clean-os",
            "macos-installer-clean-os",
        ...
        self.assertTrue(
            all(item["runtimeResult"] != "PASS" for item in matrix["targets"])
        )
    ...
    def test_macos_signing_refreshes_signature_dependent_wrapper_manifests(self):
        ...
        refresh = (ROOT / "scripts/refresh_phase13a_wrapper_manifests.py").read_text(
            encoding="utf-8"
        )
        manifests = (ROOT / "tools/phase13a/distribution_manifest.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("refresh_phase13a_wrapper_manifests.py", signing)
        self.assertIn("${wrappers[@]}", signing)
    ...
    def test_wrapper_compatibility_patches_are_pinned_and_applied(self):
        ...
        compatibility = (
            ROOT
            / "packaging/phase13a/patches/clap-wrapper-vst3-editor-compatibility.patch"
    ...
    def test_wrapper_clap_path_patch_is_pinned_and_applied(self):
        build = (ROOT / "scripts/build_phase13a_formats.py").read_text(encoding="utf-8")
        clap_path_patch = (
            ROOT / "packaging/phase13a/patches/clap-wrapper-clap-path.patch"
        ).read_text(encoding="utf-8")
    ...
    def test_wrapper_contract_requires_modern_windows_package_and_offline_preflight(
        self,
    ):
        wrapper = (
    ...
    def test_format_builder_discovers_vst3_in_declared_artifact_output(self):
        materializer = (ROOT / "tools/phase13a/payload_materializer.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("first_artifact_from(", materializer)
        self.assertIn(
            '(wrapper_build, output), "ProjectSEAMEditor.vst3", configuration',
            materializer,
        )

    def test_installer_verifier_reads_the_packaged_trust_root_at_configuration(self):
        cmake = (ROOT / "CMakeLists.txt").read_text(encoding="utf-8")
        trust = json.loads(
            (ROOT / "packaging/trust/update-root-public-key.json").read_text(
                encoding="utf-8"
            )
        )
        self.assertIn("packaging/trust/update-root-public-key.json", cmake)
        self.assertIn("string(JSON SEAM_INSTALLER_ROOT_PUBLIC_KEY_HEX GET", cmake)
        self.assertNotIn(trust["publicKey"], cmake)

    def test_offline_documentation_manifest_is_bundled_by_format_builder(self):
        build = (ROOT / "scripts/build_phase13a_formats.py").read_text(encoding="utf-8")
        inputs = (ROOT / "tools/phase13a/release_inputs.py").read_text(encoding="utf-8")
        self.assertIn("materialize_supporting_files(", build)
        self.assertIn("external-beta-documentation.json", inputs)
        self.assertIn("Documentation", inputs)

    def test_separate_future_validation_document_is_mandatory(self):
        text = (ROOT / "docs/phase13a/MANDATORY_FUTURE_VALIDATION_KO.md").read_text(
            encoding="utf-8"
        )
        for phrase in (
            "반드시",
    ...
    def test_windows_installer_uses_pinned_permissive_nsis_path(self):
        script = (ROOT / "packaging/windows/ProjectSEAM.nsi").read_text(
            encoding="utf-8"
        )
        ...
        lock = json.loads(
            (ROOT / "phase13a/tool-lock.json").read_text(encoding="utf-8")
        )
    ...
    def test_clean_installer_tests_write_machine_readable_evidence(self):
        windows = (ROOT / "scripts/test_windows_installer.ps1").read_text(
            encoding="utf-8"
        )
    ...
    def test_macos_payload_signing_defers_gatekeeper_to_notarized_pkg(self):
        ...
        notarize = (ROOT / "scripts/notarize_macos_installer.sh").read_text(
            encoding="utf-8"
        )
        self.assertIn("spctl --assess --type install", notarize)

    def test_dependency_fetcher_is_self_contained_and_exact(self):
        text = (ROOT / "scripts/fetch_phase13a_dependencies.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_format_builder_handles_macos_bundle_resources(self):
        text = (ROOT / "tools/phase13a/payload_materializer.py").read_text(
            encoding="utf-8"
        )
        build = (ROOT / "scripts/build_phase13a_formats.py").read_text(encoding="utf-8")
        targets = (ROOT / "tools/phase13a/release_payload.py").read_text(
            encoding="utf-8"
        )
        self.assertIn('Contents" / "Resources', text)
        self.assertIn("return destination, resources", text)
        self.assertLess(
            build.index("materialize_identity_sidecars(output, payload_platform)"),
            build.index("clap_sha256 = tree_sha256(clap_output)"),
        )
