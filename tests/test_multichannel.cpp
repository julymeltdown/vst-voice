...
TEST_CASE("current project schema persists buses sends and track output matrix") {
  ...
  CHECK(encoded);
  CHECK(encoded.value().find("\"schemaVersion\": " + std::to_string(seam::formats::ProjectJsonCodec::kSchemaVersion)) != std::string::npos);
