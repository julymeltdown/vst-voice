...
core::Result<void> validatePoints(std::span<const GrowlAutomationPoint> points) {
  if (points.size() > kMaximumGrowlPoints) {
    return core::failure(core::ErrorCode::InvalidArgument,
                         "Growl curve exceeds the supported point count");
  }
  ...
  for (const auto& point : points) {
    ...
    if (previous.has_value() && point.tick <= *previous) {
      return core::failure(core::ErrorCode::InvariantViolation,
                           "Growl points must be strictly ordered and unique");
    }
    previous = point.tick;
  }
  return core::success();
...
core::Result<void> GrowlAutomationPoint::validate() const {
  if (tick < time::Tick{0} || !std::isfinite(amount) || amount < 0.0F ||
      amount > kMaximumGrowl) {
    return core::failure(core::ErrorCode::InvariantViolation,
                         "Growl requires nonnegative time and a finite amount between zero and one");
  }
  return core::success();
}

core::Result<void> GrowlAutomation::validate() const {
  return validatePoints(points_);
}

core::Result<void> GrowlAutomation::replacePoints(
    std::vector<GrowlAutomationPoint> points) {
...
core::Result<void> GrowlAutomation::upsert(GrowlAutomationPoint point) {
  const auto valid = point.validate();
  if (!valid) return valid;
  const auto iterator = std::lower_bound(
      points_.begin(), points_.end(), point.tick,
      [](const GrowlAutomationPoint& candidate, time::Tick tick) {
        return candidate.tick < tick;
      });
  if (iterator != points_.end() && iterator->tick == point.tick) {
    *iterator = point;
  } else {
    if (points_.size() >= kMaximumGrowlPoints) {
      return core::failure(core::ErrorCode::InvalidArgument,
                           "Growl curve exceeds the supported point count");
    }
...
bool GrowlAutomation::erase(time::Tick tick) noexcept {
  const auto iterator = std::lower_bound(
      points_.begin(), points_.end(), tick,
      [](const GrowlAutomationPoint& point, time::Tick value) {
...
float GrowlAutomation::valueAt(time::Tick tick) const noexcept {
  ...
  const auto right = std::upper_bound(
      points_.begin(), points_.end(), tick,
      [](time::Tick value, const GrowlAutomationPoint& point) {
