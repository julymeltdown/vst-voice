...
core::Result<void> validatePoints(std::span<const BreathinessAutomationPoint> points) {
  if (points.size() > kMaximumBreathinessPoints) {
    return core::failure(core::ErrorCode::InvalidArgument,
                         "Breathiness curve exceeds the supported point count");
  }
  ...
  for (const auto& point : points) {
    ...
    if (previous.has_value() && point.tick <= *previous) {
      return core::failure(core::ErrorCode::InvariantViolation,
                           "Breathiness points must be strictly ordered and unique");
    }
    previous = point.tick;
  }
  return core::success();
...
core::Result<void> BreathinessAutomationPoint::validate() const {
  if (tick < time::Tick{0} || !std::isfinite(amount) || amount < 0.0F ||
      amount > kMaximumBreathiness) {
    return core::failure(core::ErrorCode::InvariantViolation,
                         "Breathiness requires nonnegative time and a finite amount between zero and one");
  }
  return core::success();
}

core::Result<void> BreathinessAutomation::validate() const {
  return validatePoints(points_);
}

core::Result<void> BreathinessAutomation::replacePoints(
    std::vector<BreathinessAutomationPoint> points) {
...
core::Result<void> BreathinessAutomation::upsert(BreathinessAutomationPoint point) {
  const auto valid = point.validate();
  if (!valid) return valid;
  const auto iterator = std::lower_bound(
      points_.begin(), points_.end(), point.tick,
      [](const BreathinessAutomationPoint& candidate, time::Tick tick) {
        return candidate.tick < tick;
      });
  if (iterator != points_.end() && iterator->tick == point.tick) {
    *iterator = point;
  } else {
    if (points_.size() >= kMaximumBreathinessPoints) {
      return core::failure(core::ErrorCode::InvalidArgument,
                           "Breathiness curve exceeds the supported point count");
    }
...
bool BreathinessAutomation::erase(time::Tick tick) noexcept {
  const auto iterator = std::lower_bound(
      points_.begin(), points_.end(), tick,
      [](const BreathinessAutomationPoint& point, time::Tick value) {
...
float BreathinessAutomation::valueAt(time::Tick tick) const noexcept {
  ...
  const auto right = std::upper_bound(
      points_.begin(), points_.end(), tick,
      [](time::Tick value, const BreathinessAutomationPoint& point) {
