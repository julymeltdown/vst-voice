...
enum class ArticulationGestureKind { OralVowel, Frication, Nasal, Plosive, VoicedFrication };
[[nodiscard]] inline bool isVoicedGesture(ArticulationGestureKind kind) noexcept {
  return kind == ArticulationGestureKind::OralVowel || kind == ArticulationGestureKind::Nasal || kind == ArticulationGestureKind::VoicedFrication;
}
...
struct PlosiveBinding final { std::string phone; FricationConfig source; double burstMilliseconds{10.0}; };
...
public:
  static constexpr std::uint32_t algorithmRevision = 8U;
  ...
  [[nodiscard]] static core::Result<ArticulationPlan> compileRecipe(
      ...
      std::stop_token stop = {}, bool allowVoicedFrication = true);
